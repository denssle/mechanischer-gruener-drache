...
## Reaction-Role-Feature (`/rolle`)

- `/rolle hinzufuegen message-id:<id> emoji:<emoji> rolle:<@Rolle>` (Admin) verknüpft eine Nachricht+Emoji-Kombination mit einer Rolle; `/rolle entfernen` löst sie wieder. Mehrere Bindungen (auch mehrere Emojis auf derselben Nachricht) sind möglich, Storage ist ein simpler Redis-Key pro Bindung (`REACTIONROLE:{messageId}:{emojiKey}`), kein Hash nötig.
- Ursprünglicher Use-Case: Regel-Akzeptanz beim Server-Beitritt. Verhalten ist **symmetrisch** (Reaction weg = Rolle weg) – bewusste Entscheidung vom 2026-07-04, nicht die einzig mögliche Option (Alternative wäre "einmalig vergeben, Entfernen der Reaction ändert nichts mehr").
- `/rolle hinzufuegen` muss im **selben Channel** wie die Ziel-Nachricht ausgeführt werden (`interaction.channel.messages.fetch(messageId)` – es gibt keine serverweite Message-Suche über die Discord-API). Der Bot reagiert beim Anlegen automatisch selbst mit dem Emoji auf die Nachricht, damit User sehen, worauf sie klicken müssen.
- Emoji-Matching-Key: Custom-Emojis (`<:name:id>` bzw. `<a:name:id>`) werden auf ihre ID reduziert (stabil, name kann sich ändern), Standard-Unicode-Emojis werden 1:1 als Key verwendet. Muss zwischen Command-Input (String-Option, User tippt/fügt ein) und live ankommenden `reaction.emoji` (`.id ?? .name`) konsistent geparst werden – beide Seiten nutzen dieselbe `toEmojiKey`-Funktion in `reactionRole.handler.ts`.
- Braucht `GatewayIntentBits.GuildMessageReactions` sowie `Partials.Reaction`/`Partials.User` zusätzlich zu den schon vorhandenen Partials (`client.ts`) – ohne die feuern `MessageReactionAdd`/`-Remove` für alte/nicht gecachte Nachrichten nicht, was hier besonders kritisch ist: die Regeln-Nachricht muss ja auch nach Bot-Neustarts noch funktionieren.
- Setzt voraus, dass der Bot "Rollen verwalten"-Rechte hat **und** seine eigene Rolle in der Server-Hierarchie über der zu vergebenden Rolle steht – sonst schlägt `member.roles.add/remove` mit einem Discord-API-Fehler fehl (wird geloggt, crasht nicht, aber die Rolle wird dann nie vergeben). Reine Server-Konfiguration, kein Code-Fix möglich.
- `reactionRole.handler.ts` importiert `client` **nicht** (nur die Discord.js-Objekte aus den Event-Parametern selbst) – damit fällt die zirkuläre-Import-Falle (siehe Stolperfallen) hier von vornherein weg.
