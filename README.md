...
- [ ] **Morgengruß-Emojis als Tabelle auf `/config`** (aus dem Review 2026-07-26): Wer welches persönliche Emoji hat, ist derzeit nirgends einsehbar – der Lern-Button meldet nur „X Emojis gelernt", welche Zuordnung dabei herauskam, sieht man erst, wenn jemand am nächsten Morgen schreibt. Gewünscht ist eine schlichte Übersicht **Name → Emoji** im Morgengruß-Bereich, direkt unter dem Lern-Button. Datenquelle ist der vorhandene Redis-Hash `GREETING:EMOJI` (userId → Emoji), Namensauflösung über den Guild-Cache wie in `holeMitglieder` (`config.settings.ts`). **Drei Punkte vorher klären:** (1) **Custom-Emojis liegen als blanke ID im Hash** (`emoji.id ?? emoji.name`, siehe `werteReaktionenAus`) – die Tabelle muss sie über `guild.emojis.cache.get(id)` zu `<:name:id>` auflösen, sonst steht dort nur eine Zahl; Unicode-Emojis stehen direkt als Zeichen drin und können so raus. (2) **Gelernt vs. abgeleitet unterscheiden:** wer nichts Gelerntes hat, wird per `ableiteEmoji(userId)` bedient – zeigt die Tabelle nur den Hash, fehlt die halbe Belegschaft, zeigt sie alle ohne Kennzeichnung, sieht der Hash-Eintrag aus wie der Fallback. Also beide Spalten/Fälle markieren. (3) Ob man eine Zuordnung **von Hand überschreiben** können soll (Dropdown + Speichern statt nur Anzeige) – das ist der Unterschied zwischen einer Anzeige und einem weiteren Formular samt Whitelist/CSRF; im Zweifel erst read-only bauen. Reine Anzeige, keine neue Redis-Struktur. Hängt inhaltlich am Befund, dass der Morgengruß **nirgends in einer Hilfe erklärt** wird – falls das mit angefasst wird: bewusst entscheiden, ob die Funktion eine stille Spielerei bleiben soll oder einen Satz in `HELP_TEXT` bekommt.
...
- [ ] Täglicher **13:37-Anstupser als Abo**: Wer will, meldet sich selbst an und bekommt dann jeden Tag um 13:37 (lokale Host-Zeit, Europe/Berlin) eine **DM** vom Bot: „Er brauchts mal wieder". **Opt-in ist die tragende Design-Entscheidung** – niemand wird ungefragt angeschrieben, und die DM-Schwäche (Discord verweigert Bot-DMs mit Fehler `50007`, wenn „Direktnachrichten von Servermitgliedern" aus ist) trifft nur Leute, die sich aktiv angemeldet haben. Deshalb bei der Anmeldung **direkt eine Test-DM** schicken: scheitert sie, sagt die (ephemere) Antwort sofort „deine DMs sind zu" – sonst wartet jemand wochenlang still auf nichts
  - Befehl: für **alle** offen (kein Admin), Gruppen-Command mit `an`/`aus`/`status`. Teilnehmerliste als Redis-**Set** (`sAdd`/`sRem`/`sMembers`), nur User-IDs – gehört in `docs/datenhaltung.md`
  - Scheduling: den **vorhandenen** `setInterval` (60 s) in `index.ts` samt Tagesmarker-Muster des Mitternachts-Posts (`SPORT:LAST_DAILY_POST`) wiederverwenden, keinen zweiten Mechanismus/keine Cron-Dependency bauen: Timer stupst jede Minute an, der Handler prüft selbst `Stunde === 13 && Minute === 37` plus einen eigenen Tagesmarker (`YYYY-MM-DD`) gegen Doppel-DMs
  - **Verpasste Tage nicht nachholen** (anders als beim Kilometerstand): war der Bot um 13:37 offline, ist ein Anstupser um 15 Uhr sinnlos – der Tagesmarker darf hier also nicht als „hole nach" gelesen werden
  - Zustellung pro Person in **eigenem try/catch** (`console.error`): eine geschlossene DM darf die DMs der anderen nicht abbrechen. Offen: ob nach wiederholtem `50007` automatisch abgemeldet wird – für den Hobby-Scope vermutlich unnötig, einfach loggen reicht
...
- [x] **Morgengruß-Tradition:** Die erste Nachricht **jeder Person** am Tag im Morgengruß-Kanal (über `/config` gesetzt) wird vom Bot nur per Reaktion begrüßt (👋 + persönliches Emoji – kein eigener Post). Doppelgruß-Schutz pro Person per Redis-Hash `GREETING:LAST_DAY` (Feld = userId). **Persönliches Emoji zweistufig:** aus der Chat-Historie **gelernt** (welches Emoji lag bei früheren Begrüßungen neben dem 👋 – `GREETING:EMOJI`), aufgefrischt per `/morgengruss lernen` (Admin); Fallback ist ein aus der User-ID abgeleitetes Pool-Emoji. Auto-Listener `greeting.handler.ts`, `werteReaktionenAus`/`ableiteEmoji` exportiert + getestet
