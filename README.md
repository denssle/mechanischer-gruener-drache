...
- [ ] Sport: Jede Nacht um Mitternacht die gemeinsame Gesamt-Kilometerzahl automatisch in den Sport-Kanal (`SPORT:ANNOUNCEMENT_CHANNEL`) posten – täglicher Stand ohne dass jemand `/sport gesamt` aufrufen muss. **Zu klären vor dem Bau: Es gibt bislang KEINE Scheduling-Mechanik im Projekt** (kein `setInterval`/`cron`, Runtime-Deps sind nur `discord.js`/`express`/`redis`) – empfohlen ein `setInterval` in `index.ts` mit Tagesabgleich statt neuer Dependency (`node-cron`); Host läuft auf Europe/Berlin, „Mitternacht lokal" passt also direkt. Posten wie bei den Meilensteinen (`announceReachedMilestones`-Muster). **Doppelpost-Schutz nötig:** ein „zuletzt-gepostet"-Tagesmarker in Redis (z.B. `SPORT:LAST_DAILY_POST`, `YYYY-MM-DD` – gleiches Muster wie der Morgengruß-Marker), sonst postet ein Tick kurz nach einem Neustart evtl. doppelt
...
- [ ] Morgengruß-Tradition mitmachen: Postet jemand morgens einen Gruß in den allgemeinen Chat, reagieren alle mit :wave: plus einem **personenspezifischen Emoji** (z.B. Tirsis → :sunny:). Der Bot soll mitbegrüßen und ebenfalls **nur per Reaktion** (`message.react`, passt zum Blåhaj-/Sport-Auto-Listener-Muster – kein eigener Post) mit :wave: (+ dem passenden Emoji der schreibenden Person) mitreagieren. **Erkennung bewusst simpel:** die **erste Nachricht des Tages** im Kanal, egal welcher Inhalt (kein Schlüsselwort-Matching, kein Uhrzeitfenster). Braucht also einen „letzter-Grußtag"-Marker in Redis (z.B. `GREETING:LAST_DAY`, Datum als `YYYY-MM-DD`) – nur wenn die aktuelle Nachricht der erste Eintrag eines neuen Tages ist, wird reagiert. Kanal wie beim Sport-Feature per Admin-Command setzen. **Offen:** Zuordnung Person→Emoji – automatische Ableitung wäre charmant (noch zu überlegen wie), sonst konfigurierbare Map (Redis-Hash User-ID → Emoji). Ohne hinterlegtes Emoji nur :wave:
