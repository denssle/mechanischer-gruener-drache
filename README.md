...
- [ ] **Morgengruß-Emojis: Unicode-Shortcodes wie `:cookie:` akzeptieren.** Discord gibt Emojis an vielen Stellen als Shortcode-Text aus (`:guillotine:`, `:wave:`) – kopiert man das ins Eingabefeld auf `/config/morgengruss-emojis`, greift `deuteEmojiEingabe` (`config.settings.ts`) nur bei **Server-Emojis**: `:name:` wird gegen `guild.emojis.cache` aufgelöst, `<:name:id>` ebenso. Ein Shortcode für ein **Standard-Unicode-Emoji** findet dort nichts → `null` → 400, obwohl die Eingabe für Nutzende völlig richtig aussieht. Fehlt also eine **Shortcode→Zeichen-Tabelle** (`cookie`→🍪). Vor dem Bau zu klären: (1) **Woher die Tabelle?** Eine Dependency (z.B. `emojibase`/`node-emoji`) widerspricht dem Minimal-Deps-Stil des Projekts (HTML-Parsing, Cookies, OAuth sind alle von Hand) und schleppt für ein Randfeature ein paar tausend Namen mit – Alternative ist eine **kleine handgepflegte Liste in `src/data/`** (Muster: `tipps.ts`/`nettigkeiten.ts`), die die gängigen Fälle abdeckt und sonst weiter sauber ablehnt. (2) **Reihenfolge in `deuteEmojiEingabe`:** Server-Emoji zuerst prüfen, Unicode-Tabelle nur als Fallback – ein gleichnamiges Server-Emoji soll gewinnen, sonst ändert sich stillschweigend das Verhalten bestehender Zuordnungen. (3) `eingabeFuer` (die Umkehrung, füllt das Feld vor) bleibt beim **Zeichen** – der Shortcode ist ein Eingabeweg, keine Anzeigeform. Randfall bewusst außen vor: Custom-Emojis **fremder** Server (`<:x:id>` aus einer anderen Guild) bleiben abgelehnt – der Bot müsste dort Mitglied sein, sonst scheitert `message.react` später still.
...
- [ ] **Ping-Pong-Seasons:** Die Punkte laufen monatsweise – am Monatsende bekommt Platz eins eine Auszeichnung, danach werden die Punkte zurückgesetzt. Zweck: die Bestenliste soll nicht ewig von Bestandsspielern zementiert sein, jeder Monat startet neu. Umsetzung naheliegend über den **vorhandenen 60-s-`setInterval` in `index.ts`** (kein zweiter Mechanismus, kein Cron) mit Redis-Monatsmarker analog `SPORT:LAST_DAILY_POST`/`ANSTUPSER:LAST_DAY` – der Handler entscheidet selbst, ob die Season schon abgerechnet wurde. **Entschieden:**
  - **Auszeichnung = Discord-Rolle** („aktueller Champion", vom Bot vergeben und dem Vormonats-Sieger wieder abgenommen – die Rolle zeigt also immer *den amtierenden* Champ, sie sammelt sich nicht an). Braucht Rollen-verwalten-Recht + die Bot-Rolle **über** der Champion-Rolle in der Hierarchie, wie bei `/rollenbutton`; die Rollen-ID wird über **`/config`** gesetzt (Rollen-Dropdown existiert dort schon für die Twitch-Rolle, Muster übernehmen). Ohne gesetzte/vergebbare Rolle darf die Abrechnung nicht scheitern – nur loggen und trotzdem resetten.
  - **Dazu eine Ruhmeshalle mit eigenem Befehl** (`/pingpong ruhmeshalle`), weil die Rolle immer nur den aktuellen Champ zeigt und die Vormonate sonst spurlos verschwinden: Redis-Eintrag Monat→Sieger(+Punktestand), Liste absteigend nach Monat.
  - **Kein Verkündungskanal** – der Bot postet die Season nicht von sich aus, wer wissen will, wer gewonnen hat, fragt `/pingpong ruhmeshalle` ab (passt zur Noise-Linie: kein ungefragter Post für alle).
  - **Reset betrifft nur den Score.** `PING_PONG:SERIE`/`REKORD` bleiben unangetastet (Rekord ist eine persönliche Bestmarke, die laufende Serie hängt am Spielverhalten, nicht an der Saison).
  - **Bei Gleichstand entscheidet das Los** (Zufallsauswahl unter den Punktgleichen – bewusst simpel, kein Tie-Break über Serie o.ä.). Die Auswahl gehört in eine reine, testbare Funktion.
  - **Leere Season** (niemand hat gespielt): kein Sieger, kein Ruhmeshallen-Eintrag, Rolle bleibt wie sie ist.
  - Achtung: der Score-Key ist aus Legacy-Gründen formatgebunden (`userId + REDIS_KEYS.PING_PONG`) – beim Reset nicht das Key-Format ändern.
...
- [ ] **Neuer-Tag-Countdown** („der neue Tag bricht in 3 h 34 m an, dann füllt sich der Wald wieder") – war als Modul-Idee zurückgestellt, ist seit der Prüfung vom 2026-07-28 (siehe unten) **ohne Modul machbar**: `about.php?op=setup` zeigt die Restzeit bis zum Tageswechsel öffentlich im Klartext. Umsetzung wie `/news`/`/online`: Abruf über `fetchLotgdHtml` (ISO-8859-1 + Bot-UA sind dort schon gekapselt), reine exportierte Parse-Funktion mit Tests, **fehlertolerant** (`null` statt Crash, wenn lotgd.de sein Markup ändert – Scraping ist fragil by design). Vor dem Bau zu klären: (1) **Eigener flacher Befehl** (z.B. `/tageswechsel`, dann Pflicht-Eintrag im `HELP_TEXT` + `/spielwelt`) **oder** eine Zeile in einem bestehenden Spielwelt-Befehl (`/online` – dort passt „lohnt sich der Wald noch heute?" inhaltlich am besten und es entsteht kein zusätzlicher Befehl). (2) **Zweiter Fetch oder nicht:** Die Info steht auf `about.php`, nicht auf `list.php` – hängt man sie an `/online`, kostet das einen zweiten Abruf pro Aufruf; ein kurzer In-Memory-Cache (Muster `ROSTER_CACHE_MS` in `character.service.ts`) wäre der Ausgleich, die Restzeit ändert sich ohnehin nur minütlich. (3) **Ausgabeform:** die fertige Klartext-Restzeit übernehmen oder in Discord-Timestamps (`<t:unix:R>`) umrechnen, die sich pro Betrachter selbst aktualisieren – letzteres ist schöner, braucht aber `Aktuelle Server Zeit` als Bezug und damit eine Annahme über die Server-Zeitzone.
