...
Die Admin-**Einstellungen** werden nicht mehr per Slash-Befehl gesetzt, sondern über die **Web-Konfigurationsseite `/config`** (Discord-Login, nur Server-Admins): die Kanäle (Protokoll, Twitch-Benachrichtigung, Sport-Ankündigung, Morgengruß) + Twitch-Benachrichtigungsrolle, der **Event-Termin** sowie die **Sport-Admin-Funktionen** (Kilometerstand eines Mitglieds setzen, Bestandskilometer, Meilensteine anzeigen/entfernen). Dafür entfallen sind `/protokoll`, `/twitch benachrichtigungskanal`/`benachrichtigungsrolle`, `/sport ankuendigungskanal`/`setzen`/`altkilometer`/`altkilometer-setzen`/`meilenstein liste`/`meilenstein entfernen`, `/morgengruss kanal` und `/event setzen`/`entfernen`. Als Befehle bleiben u.a. `/morgengruss lernen`, `/event countdown` und `/sport meilenstein setzen` (bewusst für alle offen).
