...
- [x] **Morgengruß-Tradition:** Die erste Nachricht **jeder Person** am Tag im Morgengruß-Kanal (über `/config` gesetzt) wird vom Bot nur per Reaktion begrüßt (👋 + persönliches Emoji – kein eigener Post). Doppelgruß-Schutz pro Person per Redis-Hash `GREETING:LAST_DAY_BY_USER` (Feld = userId). **Persönliches Emoji zweistufig:** aus der Chat-Historie **gelernt** (welches Emoji lag bei früheren Begrüßungen neben dem 👋 – `GREETING:EMOJI`), aufgefrischt per Button auf `/config`; Fallback ist ein aus der User-ID abgeleitetes Pool-Emoji. Wer welches Emoji hat, zeigt die Übersicht **Name → Emoji → Herkunft** auf `/config`. Auto-Listener `greeting.handler.ts`, `werteReaktionenAus`/`ableiteEmoji` exportiert + getestet
