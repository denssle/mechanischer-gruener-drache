...
| `/pingpong ansageduell` | Duell mit vorheriger Ansage – trifft sie zu, gibt es einen Extra-Punkt |
...
- [ ] Idee: **„Schere, Stein, Papier"-Duell als eigener, alternativer Befehl** (z.B. `/pingpong taktikduell`) – das normale `/pingpong herausfordern` bleibt daneben unangetastet, die Mechanik muss also nicht ins bestehende Duell hineingebaut werden. Beide Seiten wählen verdeckt eine Aktion (z.B. Schmetterball/Konter/Lupfer), die Kombination entscheidet. **Zustandslos nach dem bestehenden Muster:** der Herausforderer wählt seine Aktion schon als Option beim Befehl, sie wandert mit in die Button-`customId` (wie heute die User-IDs); statt „Annehmen" gibt es drei Aktions-Buttons, der eine Klick des Herausgeforderten löst das ganze Match auf – kein Redis, Neustart-fest. Zu klären beim Bauen: Gleichstand (beide wählen dasselbe) → Rückfall auf den 50/50-Ballwechsel; und bewusst hinnehmen, dass die Wahl zwar im Client unsichtbar ist (Optionen/`customId`s werden anderen nicht angezeigt), technisch Versierte sie aber aus dem API-Payload lesen könnten – für eine Community-Spielerei okay
- [x] **Ansage-Duell** (`/pingpong ansageduell gegner:@user ansage:<Ich gewinne|Ich verliere>`) als eigener Befehl neben dem normalen `/pingpong herausfordern`: Der Herausforderer sagt vorher an, wie es ausgeht; gespielt wird weiter rein zufällig, aber eine **getroffene Ansage bringt einen Extra-Punkt**. Eine falsche Ansage kostet bewusst nichts zusätzlich (Sieg/Niederlage regeln die Punkte, die Ansage kann nur belohnen) – eine angesagte Niederlage gleicht den Punktverlust also gerade aus. Die Siegesserie richtet sich weiter nach dem echten Spielausgang. Zustandslos wie das normale Duell: die Ansage steckt mit in der Button-`customId`, kein Redis
