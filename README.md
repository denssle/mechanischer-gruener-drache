...
Die Admin-**Einstellungen** (Protokoll-Kanal, Twitch-Benachrichtigungskanal + -rolle, Sport-Ankündigungskanal, Morgengruß-Kanal sowie der **Event-Termin**) werden nicht mehr per Slash-Befehl gesetzt, sondern über die **Web-Konfigurationsseite `/config`** (Discord-Login, nur Server-Admins). Die früheren Befehle `/protokoll`, `/twitch benachrichtigungskanal`/`benachrichtigungsrolle`, `/sport ankuendigungskanal`, `/morgengruss kanal` und `/event setzen`/`entfernen` sind dafür entfallen; `/morgengruss lernen` (Emoji-Auffrischung) und `/event countdown` bleiben.
