...
- [ ] Idee: **Ansage-Duell als eigener, alternativer Befehl** (z.B. `/pingpong ansageduell`) – das normale `/pingpong herausfordern` bleibt daneben unangetastet. Wer herausfordert, sagt vorher an, ob er gewinnt oder verliert; das Match läuft weiter rein zufällig wie bisher, aber wer die eigene Ansage trifft, bekommt einen **Extra-Punkt**. Zustandslos nach dem bestehenden Muster: die Ansage wandert wie die User-IDs in die Button-`customId`, kein Redis. Zu klären beim Bauen: ob eine falsche Ansage auch etwas kostet (Vorschlag: nein – Sieg/Niederlage regeln die Punkte, die Ansage kann nur belohnen), und wie die Niederlagen-Ansage sich zur Siegesserie verhält
