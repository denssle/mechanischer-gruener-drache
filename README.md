...
Die Admin-**Kanal- und Rollen-Einstellungen** (Protokoll-Kanal, Twitch-Benachrichtigungskanal + -rolle, Sport-Ankündigungskanal, Morgengruß-Kanal) werden nicht mehr per Slash-Befehl gesetzt, sondern über die **Web-Konfigurationsseite `/config`** (Discord-Login, nur Server-Admins). Die früheren Befehle `/protokoll`, `/twitch benachrichtigungskanal`/`benachrichtigungsrolle`, `/sport ankuendigungskanal` und `/morgengruss kanal` sind dafür entfallen; `/morgengruss lernen` (Emoji-Auffrischung) bleibt.
...
- [ ] Verwaltungsseite `/config` – restliche Admin-Befehle migrieren (Ziel: **je weniger Admin-Befehle in Discord, desto übersichtlicher**). **Schon erledigt:** Discord-OAuth-Login (nur Server-Admins), read-only-Übersicht aller Einstellungen, bearbeitbar sind Protokoll-/Twitch-Benachrichtigungs-/Sport-Ankündigungs-/Morgengruß-Kanal + Twitch-Benachrichtigungsrolle (die zugehörigen Slash-Befehle wurden entfernt). Noch auf die Seite bringen und danach als Slash-Command entfernen:
  - **Event** (`/event setzen` / `entfernen`): wird schon read-only angezeigt, noch nicht bearbeitbar. Kein Dropdown – Datum/Uhrzeit/optionaler Titel als Eingabefelder; `parseGermanDateTime` aus `event.handler.ts` (TT.MM.JJJJ + optional HH:MM, lehnt Vergangenes/Ungültiges ab, exportiert + getestet) wiederverwenden **oder** native `<input type="date">`/`<input type="time">`. Plus einen „Event entfernen"-Weg.
  - **Sport-Admin:** `/sport setzen` (Kilometer eines Mitglieds – braucht eine Mitglied-Auswahl + Zahl; Mitgliederliste ggf. lang → überlegen: Dropdown vs. ID-Eingabe), `/sport altkilometer` (addiert) + `/sport altkilometer-setzen` (setzt, `0` = entfernen), `/sport meilenstein liste` + `entfernen` (Liste mit Entfernen-Buttons). **`/sport meilenstein setzen` bleibt** als Command (bewusst für ALLE offen, kein Admin). Alle Setter existieren in `sport.service.ts`.
  - **`/morgengruss lernen`:** als Button „Emojis aus der Historie lernen" auf `/config` (löst `greetingHandler.lerneUndSpeichere` aus). Aktion, kein gespeicherter Wert – der Scan kostet mehrere API-Calls (Request dauert kurz), Rückmeldung „X Emojis gelernt".
...
- [ ] Bot-Logs auf `/config` einsehbar machen (Erweiterung der Verwaltungsseite; die Admin-Auth via Discord-OAuth und die read-only-Anzeige existieren bereits): eine Ansicht der letzten Log-Zeilen im Browser, damit man bei „irgendwas kommt nicht an" nicht erst per SSH ins Supervisor-Log schauen muss. **Zwei Quellen denkbar:** (a) die von Uberspace/Supervisor persistierte stdout/stderr-Logdatei des `drache`-Prozesses direkt lesen (überlebt Neustarts, aber der Bot muss den Pfad kennen und Dateizugriff haben) oder (b) ein In-Memory-Ringpuffer der letzten N Zeilen im Bot selbst (`console.log/error` abfangen; einfacher, aber bei jedem Neustart leer – ausgerechnet die bei Boot-Problemen interessantesten Zeilen fehlen dann). **Datenschutz beachten:** Die Logs tragen bewusst **keine Nachrichteninhalte** (dokumentierte Linie in `docs/datenhaltung.md`), aber sehr wohl User-IDs, Fehler-Stacktraces und Kanal-IDs – auf der Admin-only-Seite vertretbar, aber vorher sicherstellen, dass **keine Secrets** (Tokens, `config.json`-Werte) ins Log geraten und damit sichtbar würden. Nur für eingeloggte Admins, wie die restliche `/config`-Seite
...
- [ ] Gesamtkilometer **gerundet** anzeigen: kaufmännisch auf ganze Kilometer (bis `,49` ab-, ab `,50` aufrunden, also `Math.round`). Betrifft die Anzeige-Stellen der Gesamtsumme (`sport.handler.ts`: Eintrags-Embed „gemeinsam schon **X km**", `/sport gesamt`, der tägliche Mitternachts-Post) – **nur die Ausgabe runden, nicht den gespeicherten Wert**, sonst gehen bei jedem Eintrag Nachkommastellen verloren und die Summe driftet. Offen: ob die Meilenstein-Schwellenprüfung (`checkAndMarkReachedMilestones`) weiterhin mit dem exakten Wert rechnet (naheliegend: ja, sonst würde eine Schwelle bei `1999,5 km` schon als erreicht gelten, obwohl sie es nicht ist)
