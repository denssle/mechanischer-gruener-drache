...
- [ ] Idee: Ping-Pong-Duell um eine **„Schere, Stein, Papier"-Mechanik** erweitern – beide Seiten wählen verdeckt eine Aktion (z.B. Schmetterball/Konter/Lupfer), die Kombination entscheidet. **Zustandslos nach dem bestehenden Muster:** der Herausforderer wählt seine Aktion schon als Option beim `herausfordern`, sie wandert mit in die Button-`customId` (wie heute die User-IDs); statt „Annehmen" gibt es drei Aktions-Buttons, der eine Klick des Herausgeforderten löst wie bisher das ganze Match auf – kein Redis, Neustart-fest. Zu klären beim Bauen: Gleichstand (beide wählen dasselbe) → Rückfall auf den heutigen 50/50-Ballwechsel; und bewusst hinnehmen, dass die Wahl zwar im Client unsichtbar ist (Optionen/`customId`s werden anderen nicht angezeigt), technisch Versierte sie aber aus dem API-Payload lesen könnten – für eine Community-Spielerei okay
- [ ] Idee: Ping-Pong-**Siegesserie** – anzeigen, wie viele Duelle am Stück man gerade ungeschlagen ist (Zähler pro User in Redis, hochzählen bei Sieg, zurücksetzen bei Niederlage; z.B. im Duell-Ergebnis erwähnen und/oder in `/pingpong bestenliste`). Optional dazu: die längste je erreichte Serie als Rekord mitführen
