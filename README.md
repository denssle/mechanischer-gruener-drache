...
- [ ] `deploy.yml` in zwei Jobs aufteilen (`test` und `deploy`, verkettet per `needs: test`), sodass nur bei grünen Tests deployt wird. Ein rotes Deployment verhindert die aktuelle Ein-Job-Pipeline zwar schon (`npm test` läuft als Step vor `rsync`, ein roter Step bricht den Job ab) – der Gewinn ist Struktur und Sichtbarkeit: Test/Deploy sind im Actions-UI getrennt sichtbar, und der Test-Job könnte zusätzlich auf Branches/PRs laufen, wo gar nicht deployt wird
